...
/**
 * Created by ATRSnail on 2016/12/29.
 */
...
public class DragScaleImageView extends RelativeLayout{
    ...
    float scale = 1;
    private AtyTopLayout atyTopLayout;//导航条
    private int fadingHeight = 500; //滑动距离，默认设置滑动500 时导航条完全消失，根据实际需求自己设置

    private TouchEventListener touchEventListener = null;
    private BackScaleListener backScaleListener = null;
    private  View otherView;//在图片上层的其他的视图
    ...
    private void initOtherView() {
        ...
        atyTopLayout = (AtyTopLayout) otherView.findViewById(R.id.set_topbar);
...
//        } else {
//            getParent().requestDisallowInterceptTouchEvent(false);// true表示父类的不可用;
//        }
//        switch (event.getAction() & MotionEvent.ACTION_MASK) {
        switch (event.getAction() ) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                otherHeight = otherView.getY();
                currentBottom=otherView.getBottom();
                currentTop=otherView.getTop();
...
//              float dx = event.getX() - startPoint.x; // 得到x轴的移动距离
                    ...
                    if (dy > 0) {
                        ...
                        imageView.setImageMatrix(matrix);
                        currentTop=(int)(currentTop+dy);
                        currentBottom=(int)(currentBottom-dy);
                        otherView.layout(otherView.getLeft(),currentTop,otherView.getRight(),currentBottom);
                        if (dy>=displayHeight){
                            atyTopLayout.setAlpha(0);
                        }else {
                            atyTopLayout.setAlpha((1-dy/displayHeight));
                        }
                    }else {
                        atyTopLayout.setAlpha(1);
                    }
                }
                break;
            // 手指离开屏幕
